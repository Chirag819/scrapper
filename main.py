...
class ReviewScraperManager:
    ...
    SUPPORTED_SOURCES = ['capterra', 'trustpilot', 'all']  # 'g2' commented out
    ...
    def scrape_reviews(self, company_name, start_date, end_date, source='all'):
        ...
        if source == 'all':
            # Process Capterra and Trustpilot first, G2 commented out for now
            sources_to_scrape = ['capterra', 'trustpilot']  # 'g2' commented out
        else:
...
Examples:
  ...
  python main.py --company "HubSpot" --start-date "2023-01-01" --end-date "2023-12-31" --output "hubspot_reviews.json"
    ...
    parser.add_argument(
        '--source',
        default='all',
        choices=['capterra', 'trustpilot', 'all'],  # 'g2' commented out
